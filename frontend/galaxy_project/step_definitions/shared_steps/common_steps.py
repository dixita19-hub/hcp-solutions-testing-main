...
@then(parsers.re("I expect element '(?P<locator_path>.*)' has items:(?P<data_table_raw>.*)", flags=re.S))
...
def element_with_value_is_enabled(
    ...
    value = env_variables.env_formatted(inner_text)
    locator = locators.parse_and_get(locator_path, selenium_generics)
...
@then(parsers.re("The element with inner text '(?P<inner_text>.*)' and element '(?P<locator_path>.*)' has text '(?P<text>.*)'"),
      converters=dict(inner_text=env_variables.env_formatted, text=env_variables.env_formatted))
def element_with_innertext_equals_text(selenium_generics: SeleniumGenerics, locators: Locators, locator_path, inner_text, text):
        locator = locators.parse_and_get(locator_path, selenium_generics)
        locator = locator.format(inner_text)
        actual_text = selenium_generics.get_element_text(locator)
...
@when(parsers.re("I set text with current_timestamp '(?P<text>.*)' to field '(?P<locator_path>.*)'"),
      converters=dict(text=env_variables.env_formatted), )
def set_element_value(selenium_generics: SeleniumGenerics, text: str, locators: Locators, locator_path):
    if MOBILE_SUFFIX in locator_path:
        with context_manager(selenium_generics):
            selenium_generics.enter_text(locators.parse_and_get(locator_path, selenium_generics), text+str(datetime.datetime.now()))
    else:
        selenium_generics.enter_text(locators.parse_and_get(locator_path, selenium_generics), text+str(datetime.datetime.now()))
