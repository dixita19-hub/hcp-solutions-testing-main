...
def setup_basic_auth(request: FixtureRequest, username: str, password: str):
    ...
    * create an attribute `request_auth` of `request.node` object
    ...
    """
    username = env_variables.env_formatted(username)
    password = env_variables.env_formatted(password)
...
def setup_bearer_auth(request: FixtureRequest, token: str):
    ...
    * create an attribute `request_auth` of `request.node` object
    ...
    """
    token = env_variables.env_formatted(token)
...
def set_request_endpoint(request: FixtureRequest, request_name: str, base_url: str, endpoint: str, with_logs: bool = False):
    ...
    * urlappend Base Url and endpoint and assign it to "url" key of request_args of the named request.
    """
    request.node.base_url = env_variables.env_formatted(base_url)
    ...
    request.node.request_args[request_name]["url"] = utils.urlappend(
        helpers.get_base_url(request), env_variables.env_formatted(endpoint)
    )
    if with_logs:
        logger.info(
    ...
    * Header Key-Value pairs fetched from datatable would be joined with the ones specified as default
    ...
    if isinstance(headers, dict):
        for k, v in headers.items():
            headers[k] = env_variables.env_formatted(v)
    else:
...
def add_url_path_parameter(request: FixtureRequest, request_name: str, url_param: str, with_logs: bool = False):
    ...
    * urlappend the existing url and the url parameter for the specified request.
    ...
    request.node.request_args[request_name]["url"] = utils.urlappend(
        ...
        env_variables.env_formatted(url_param),
    )
    ...
    * Check for missing url or endpoint (by checking request_args attribute & request_name key in
    ...
    if isinstance(query_params, dict):
        for k, v in query_params.items():
            query_params[k] = env_variables.env_formatted(v)
    else:
    ...
    * Add query params passed in the step as "params" in request arguments.
    ...
    if isinstance(params, dict):
        for k, v in params.items():
            params[k] = env_variables.env_formatted(v)
    else:
