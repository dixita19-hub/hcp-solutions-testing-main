...
from bp_core.utils.pytest_terminal_report import pytest_terminal_summary
...
from bp_core.utils.utils import initialize_output_dirs, remove_chars_from_string, zip_screenshots_files, save_base64_as_png
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    if session.config.option.driver is not None:
        from bp_core.installation.installation_scripts.update_webdrivers import update_webdriver_if_needed
        browser = session.config.option.driver
        update_webdriver_if_needed(browser, session.config.rootdir)
...
def pytest_sessionfinish(session, exitstatus) -> None:
    ...
    if not metadata.get("Base URL", False):
        if os.getenv("BASE_URL", False):
            metadata["Base URL"] = os.getenv("BASE_URL")
        else:
...
def pytest_runtest_makereport(item, call):
    ...
    if not bp_storage.is_api_testing():
        ...
        if rep.when == "teardown":
            ...
            if bdd_example:
                if isinstance(bdd_example, dict):
            ...
            if driver is not None:
                ...
                if (rep.skipped and xfail) or (rep.failed and not xfail) or (rep.passed and not xfail):
                    ...
                    if (scenario := getattr(rep, "scenario", None)) and (feature := scenario.get('feature', None)):
                        ...
                        scenario_cache[node_name] = [element for element in scenario_cache.get(node_name, []) if element is not None]
                        if len(scenario_cache.get(node_name, [])) > 2:
                            ...
                            extra.extend(
                                [pytest_html.extras.image(screenshot) for screenshot in reversed(scenario_cache.get(node_name, []))])
                if public_link:
        ...
        if not bp_storage.is_api_testing():
            ...
            for cell in cells:
                ...
                if attr_ and len(cell) and cell[0] == "Tag":
                    tc_tag = ""
                    valid_tags = [tag for tag in report.keywords if isinstance(tag, str) and tag.startswith(TESTCASE_TAG_PREFIX)]
                    if len(valid_tags):
